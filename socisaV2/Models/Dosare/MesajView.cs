...
namespace socisaWeb
{
    public class MesajView
    {
        ...
        public MesajView(int uid, string conStr, int? _START_LIMIT, int? _END_LIMIT)
        {
            this.MesajJson = new MesajJson();
            Utilizator[] us = new List<Utilizator>().ToArray();
            this.InvolvedParties = us;
            ...
            Mesaj[] ms = (Mesaj[])u.GetMesaje(_START_LIMIT, _END_LIMIT).Result;
            /*
            ...
            */
            MesajeJson = new MesajJson[ms.Length];
            for(int i = 0; i < ms.Length; i++)
            {
                MesajeJson[i] = new MesajJson(ms[i]);
                MesajeJson[i].DataCitire = (DateTime?)ms[i].GetMessageReadDate(uid).Result;
            }
        }
    }

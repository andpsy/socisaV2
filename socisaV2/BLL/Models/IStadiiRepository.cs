    ...
    public class StadiiRepository : IStadiiRepository
    {
        ...
        public response GetAll()
        {
            try
            {
                ...
                */
                List<Stadiu> aList = new List<Stadiu>();
                MySqlDataReader r = da.ExecuteSelectQuery();
                while (r.Read())
                {
                    Stadiu a = new Stadiu(authenticatedUserId, connectionString, (IDataRecord)r);
        ...
        public response GetCombo()
        {
            try
            {
                DataAccess da = new DataAccess(authenticatedUserId, connectionString, CommandType.StoredProcedure, "STADIIsp_recursive");
                List<StadiuCombo> aList = new List<StadiuCombo>();
                MySqlDataReader r = da.ExecuteSelectQuery();
                while (r.Read())
                {
                    StadiuCombo a = new StadiuCombo((IDataRecord)r);
                    aList.Add(a);
                }
                r.Close(); r.Dispose(); da.CloseConnection();
                return new response(true, JsonConvert.SerializeObject(aList.ToArray(), CommonFunctions.JsonSerializerSettings), aList.ToArray(), null, null);

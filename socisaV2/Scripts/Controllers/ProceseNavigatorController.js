...
app.controller('ProceseNavigatorController',
    function ($scope, $http, $filter, $rootScope, $window, $timeout, $compile, $q, Upload, ngDialog) {
        ...
        $scope.init = null;
        $scope.EMPTY_ID_DOSAR = 0;

        $scope.model.CurProces = {};
        $scope.model.CurProces.Proces = {};
        $scope.model.CurProces.Proces.ID = null;
        $scope.model.CurProces.Proces.MONITORIZARE = null;
        $scope.model.procesJson = {};
        $scope.model.procesJson.Reclamant = "";
        $scope.model.procesJson.Parat = "";
        $scope.model.procesJson.Tert = "";
        ...
        $scope.TempProcesFilter.CurProces.Proces.ID = null;
        //$scope.TempProcesFilter.CurProces.Proces.MONITORIZARE = true;
        ...
        $scope.TempProcesEdit.CurProces = {};
        $scope.TempProcesEdit.CurProces.Proces = {};
        $scope.TempProcesEdit.CurProces.Proces.MONITORIZARE = true;
        $scope.TempProcesEdit.procesJson = {};
        ...
        $scope.$watch('model.procesJson.Calitate', function (newValue, oldValue) {
            if (newValue != oldValue && ($scope.searchMode == 2 || ($scope.editMode > 0 && ($rootScope.ID_DOSAR == null || $scope.EMPTY_ID_DOSAR == 1)))) {
                if (newValue == null) {
        ...
        $scope.$watch('idx', function (newValue, oldValue) {
            if (newValue != null && newValue > -1) {
                var rowId = "#proces_" + newValue;
                $(rowId).addClass("activeProces");

                if (newValue != oldValue || $scope.init == null)
                {
                    if (oldValue != null && oldValue > -1) {
                        var oldRowId = "#proces_" + oldValue;
                    ...
                    */
                    var newIdDosar = $scope.model.Procese[newValue].ID_DOSAR;
        ...
        $scope.$watch('model.procesJson.DataScaEnd', function (newValue, oldValue) {
        ...
        $scope.$watch('model.CurProces.Proces.DATA_SCA', function (newDate, oldDate) {
            if (newDate == null || newDate == undefined || newDate.length <= 10 || angular.isDate(newDate)) { return; }
            $scope.model.CurProces.Proces.DATA_SCA = $filter('date')(new Date(parseInt(newDate.substr(6))), $rootScope.DATE_FORMAT);
        });
        ...
        $scope.$on('refreshEmitEvent', function (event, data) {
            ...
            switch (data.objectType) {
                ...
                case "Reclamant":
                    ...
                    $scope.model.procesJson.Reclamant = data.object.DENUMIRE;
                    break;
                case "Parat":
                    if (data.operation == "insert") {
                        $scope.model.CurProces.Proces.ID_PARAT = data.object.ID;
                    }
                    $scope.model.procesJson.Parat = data.object.DENUMIRE;
                    break;
                case "Tert":
                    if (data.operation == "insert") {
                        $scope.model.CurProces.Proces.ID_TERT = data.object.ID;
                    }
                    $scope.model.procesJson.Tert = data.object.DENUMIRE;
                    break;
            }
            /*
        ...
        $scope.ShowInfoPortal = function (interactive) {
            if ($scope.model.CurProces.Proces.ID != $scope.lastProcesIdPortal) {
                EnableDisableInputs('#mainInfoGeneraleDosar', spinner, ACTIVE_DIV_ID, true, true);
                $.ajax({
                    async: false,
                    type: 'GET',
                    url: '/DosarePortal/Index/' + $scope.model.CurProces.Proces.ID,
                    dataType: 'html'
                }).done(function (data) {
                    var htmlContent = $compile(data)($scope);
                    $scope.lastProcesIdPortal = $scope.model.CurProces.Proces.ID;
        ...
        $scope.ShowStadii = function (interactive) {
            if ($scope.model.CurProces.Proces.ID != $scope.lastProcesIdStadii) {
                EnableDisableInputs('#mainInfoGeneraleDosar', spinner, ACTIVE_DIV_ID, true, true);
                $.ajax({
                    async: false,
                    type: 'GET',
        ...
        $scope.ShowDocumente = function (interactive) {
            if ($scope.model.CurProces.Proces.ID != $scope.lastProcesIdDocumente || $scope.docSaved != null) {
                EnableDisableInputs('#mainInfoGeneraleDosar', spinner, ACTIVE_DIV_ID, true, true);
                $.ajax({
                    async: false,
                    type: 'GET',
        ...
        */
        ...
        $scope.Afisare = function (e, sender) {
            ...
            if (!direct && now - lastkeytime <= waiting_interval && $scope.Interactive) {
                ...
                if ((direct || lastfiltervalue != filter_value) && $scope.Interactive) {
                    ...
                    ////$scope.curProcesIndex.Value = $scope.idx = 0;
                    //$scope.idx = 0;
                    ////$scope.curProcesIndex.Value = $scope.idx = null;
                    $scope.Afisare2(e);
                    lastkeytime = now;
            ...
            */
            ...
            .then(function (response) {
                if (response != 'null' && response != null && response.data != null) {
                    ...
                    if ($scope.curProcesIndex.Value == -1) {
                        $scope.curProcesIndex.Value = $scope.idx = 0;
                    }
                    $scope.ShowProces($scope.idx);
                }
        ...
        $scope.nextProces = function () {
            if ($scope.model.Procese != undefined) {
                if ($scope.curProcesIndex.Value + 1 >= $scope.RowsCount) return;
        ...
        $scope.lastProces = function () {
            if ($scope.model.Procese != undefined) {
                if ($scope.RowsCount - 1 == $scope.curProcesIndex.Value) return;

                $scope.curProcesIndex.Value = $scope.RowsCount - 1;
                if ($scope.RowsBlockIndex < Math.floor($scope.RowsCount / $scope.RowsBlockSize)) {
                    $scope.switchTempToProcesObjects();
                    $scope.RowsBlockSize = $scope.DefaultRowsBlockSize; // reinitializam RowsBlockSize pt. situatia in care au fost stergeri
                    $scope.RowsBlockIndex = Math.floor($scope.RowsCount / $scope.RowsBlockSize) - ($scope.RowsCount % $scope.RowsBlockSize == 0 ? 1 : 0);
                    $scope.idx = $scope.RowsCount % $scope.RowsBlockSize == 0 ? $scope.RowsBlockSize - 1 : $scope.RowsCount % $scope.RowsBlockSize - 1;
                    $scope.Afisare2(null);
                }
                else {
                    $scope.idx = $scope.RowsCount % $scope.RowsBlockSize == 0 ? $scope.RowsBlockSize - 1 : $scope.RowsCount % $scope.RowsBlockSize - 1;
    ...
    $scope.ShowProces = function (index) {
        ...
        $("#infoGeneraleDosar").addClass("active in");
        $scope.searchMode = 1;
        ...
        return $http.get('/Procese/Detail/' + id)
            .then(function (response2) {
                if (response2 != 'null' && response2 != null && response2.data != null) {
                    ...
                    $scope.model.CurProces.Proces.MONITORIZARE = $scope.model.CurProces.Proces.MONITORIZARE == true || $scope.model.CurProces.Proces.MONITORIZARE == false ? $scope.model.CurProces.Proces.MONITORIZARE + "" : "";
    ...
    $scope.EnterEditMode = function () {
        ...
        $scope.searchMode = 0;
        $scope.editMode = 1;
        angular.copy($scope.model.CurProces, $scope.TempProcesEdit.CurProces);
        angular.copy($scope.model.procesJson, $scope.TempProcesEdit.procesJson);
    };
        ...
        if ($rootScope.ID_DOSAR != null) {
            ...
            ngDialog.openConfirm({
                ...
                function (value) {
                    ...
                    $scope.model.CurProces.Proces.NR_SCA = $rootScope.NR_SCA;
                    $scope.model.CurProces.Proces.DATA_SCA = $rootScope.DATA_SCA;
                    $scope.model.CurProces.Proces.NR_DOSAR_CASCO = $rootScope.NR_DOSAR_CASCO;
                },
                function (reason) {
                    //trebuie sa inactivam filtrul implicit pe dosarul selectat
                    //$rootScope.ID_DOSAR = null;
                    $scope.EMPTY_ID_DOSAR = 1;
                    //$scope.Afisare2(null);
                    //$scope.model.Procese = [];
                    $scope.EnterAddModeAfterConfirm();
                });
    ...
    $scope.EnterAddModeAfterConfirm = function () {
        ...
        $scope.model.CurProces.Proces.MONITORIZARE = true;
        $scope.model.CurProces.Proces.SUMA_SOLICITATA = $scope.model.CurProces.Proces.PENALITATI = 0;
        //if ($rootScope.ID_DOSAR == null) {
        if ($scope.EMPTY_ID_DOSAR == 1) {
            $scope.model.CurProces.Proces.NR_DOSAR_CASCO = $rootScope.NR_DOSAR_CASCO = null;
            $scope.model.CurProces.Proces.ID_DOSAR = null;
            $scope.model.procesJson = {};
            $scope.model.procesJson.Calitate = null;
            $scope.$emit('NewFilterRequestEmitEvent', { newIdDosar: null, filter: null });
        }
        else {
                ...
                if (response != 'null' && response != null && response.data != null) {
                    ...
                    if ($scope.result.Status) {
                        ...
                        if ($scope.result.InsertedId != null) {
                            ...
                            $scope.$emit('refreshDashboardEmitEvent', { object: 'procese', value: 1 });
                        }
                        else {
    ...
    $scope.CancelEdit = function () {
        ...
        //if ($scope.editMode == 1) {
        angular.copy($scope.TempProcesEdit.CurProces, $scope.model.CurProces);
        angular.copy($scope.TempProcesEdit.procesJson, $scope.model.procesJson);
        $scope.EMPTY_ID_DOSAR = 0;
        //}
        /*
        if ($scope.editMode == 2) {
        ...
        */
        $scope.$emit('NewFilterRequestEmitEvent', { newIdDosar: $scope.model.CurProces.Proces.ID_DOSAR, filter: true });
            ...
            .then(function (response) {
                if (response != 'null' && response != null && response.data != null) {
                    ...
                    if ($scope.result.Status) {
                        ...
                        $scope.searchMode = 1;
                        $scope.editMode = 0;
                        $scope.model.Procese.splice($scope.idx, 1);
                        $scope.RowsCount -= 1;
                        if ($scope.idx != 0) {
                            ...
                            if ($scope.RowsBlockIndex == 0) { // eram pe prima inregistrare si am sters-o
